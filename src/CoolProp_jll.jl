...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import CoolProp_jll on Julia versions older than 1.3!")
end
    ...
    function dev_jll()
        ...
        mktempdir() do temp_env
            Pkg.activate(temp_env) do
                Pkg.develop("CoolProp_jll")
            end
        end
        # Create the override directory
        override_dir = joinpath(Pkg.devdir(), "CoolProp_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"CoolProp", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "CoolProp_jll", "src", "CoolProp_jll.jl"))
        @info("CoolProp_ll dev'ed out to /depot/dev/CoolProp_jll with pre-populated override directory")
    end
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("3351c21f-4feb-5f29-afb9-f4fcb0e27549"))
platforms = [Pkg.Artifacts.unpack_platform(e, "CoolProp", artifacts_toml) for e in artifacts["CoolProp"]]
...
if best_platform === nothing
    @debug("Unable to load CoolProp; unsupported platform $(triplet(platform_key_abi()))")
else
